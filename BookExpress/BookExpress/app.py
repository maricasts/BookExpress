...
from models import Cliente, Tecnico, Livro
...
def tecnico_dashboard():
    ...
    # Livros mais emprestados (simulado por menor estoque)
    popular_books = sorted(books, key=lambda x: x.get('estoque', 999))[:5]
    ...
    return render_template('tecnico_dashboard.html', stats=stats, books=books[:5], popular_books=popular_books)
...
def add_to_cart(book_id):
    ...
    if book and book.estoque > 0:
        ...
        session['cart'] = cart
        flash(f'"{book.titulo}" adicionado à lista de empréstimos!', 'success')
    else:
...
def remove_from_cart(book_id):
    ...
    flash('Item removido da lista de empréstimos!', 'success')
